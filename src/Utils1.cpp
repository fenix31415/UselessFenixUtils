...
namespace Utils
{
	...
	RE::NiQuaternion slerp(const RE::NiQuaternion& a_quatA, const RE::NiQuaternion& a_quatB, double a_t)
	{
		...
		if (fabs(sinHalfTheta) < 0.001) {  // fabs is floating point absolute
			result.w = (a_quatA.w * 0.5f + q2.w * 0.5f);
			result.x = (a_quatA.x * 0.5f + q2.x * 0.5f);
			result.y = (a_quatA.y * 0.5f + q2.y * 0.5f);
			result.z = (a_quatA.z * 0.5f + q2.z * 0.5f);
			return result;
		}
		float ratioA = sinf(static_cast<float>(1 - a_t) * halfTheta) / sinHalfTheta;
		float ratioB = sinf(static_cast<float>(a_t) * halfTheta) / sinHalfTheta;
	...
	void TraverseMeshes(RE::NiAVObject* a_object, bool a_bStrict, const RE::NiTransform& a_transform, std::function<void(RE::BSGeometry*, const RE::NiTransform&)> a_func)
	{
		...
		if (a_object->flags.any(RE::NiAVObject::Flag::kHidden)) {
			return;
		}
		...
		if (geom) {
			if (a_bStrict) {
				if (geom->flags.none(RE::NiAVObject::Flag::kRenderUse)) {
					return;
				}
			}
